...
An application to manage inventory items using QR codes. Users can add items with details, an image, and a list of responsible holders. The app generates a unique QR code for each item, and users can scan these codes to view or update item details.
...
- **Add Item (`/add_item`):**
    ...
    - **Dynamic Fields:** Users can add custom key-value pairs for additional details (e.g., "Warna": "Merah").
    - **Image Upload:** Main item image can be uploaded to a third-party service (imgbb.com) to get a URL.
    - **Item Holders (Pemegang Barang):**
        - Users can dynamically add one or more item holders.
        - Each holder has a name and an optional photo.
        - Holder photos are also uploaded to get a URL.
    - Saves all data, including the lists of dynamic fields and item holders, to a single Firestore document.
...
- **Item Details (`/item_details/:itemId`):**
    - Fetches and displays the full details of a specific item, including the main image.
    - **Displays Item Holders:** Shows a formatted list of all item holders, including their photos and names.
    - Provides buttons to "Edit" or "Hapus" (Delete) the item.
- **Edit Item (`/item_details/:itemId/edit`):**
    - Pre-fills a form with all existing data for an item, including dynamic fields and item holders.
    - Allows users to add, edit, or delete the item name, category, main image, dynamic fields, and the list of item holders.
    - Saves all updated data back to the corresponding Firestore document.
